...
from prop_lib import (calculate_slope,CALIBRATION_CONFIG,jsonFillFile,
                      parseInput,calibrate_setup, openAsReadJson)
from phidgetInterface import AnalogInterface
def main():
    calibrate()

def collect_calibration_data(arduino, ch_esc, ch_power):
    esc_data = []
    power_data = []
    ...
    try:
        while True:
            ...
            if line.startswith("PWM:"):
                ...
                valid,user_input = parseInput([],"Measured current (Amps) or 'q': ")
                if not valid:
                    break;
...
def calibrate():
    ...
    esc_data, power_data = collect_calibration_data(arduino, esc, power)
